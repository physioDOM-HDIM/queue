...
function Queue(config) {
	...
	this.sendToHHRPro = function () {
		...
		var search = { 'transfered': { '$exists':0}, datetime: { '$lt':moment().subtract(that.config.retry, 'm').toISOString() } };
	 ...
	 * @returns {$$rsvp$promise$$default|RSVP.Promise|*|l|Dn}
	 */
	this.relayMsg = function (type, message, record ) {
		...
		function transmitMsg( server, record ) {
			...
			return new promise( function( resolve, reject ) {
				request({
						url: server + "/api/queue/received?receivedDate="+record.datetime,
						method: "POST",
						headers: {"content-type": "text/plain"},
						rejectUnauthorized: false,
						timeout: 10000,
		...
		return new promise( function( resolve, reject) {
			...
			saveReceivedMessage( _record )
				.then( function() { 
					return checkSchema(type, message)
				})
				.then( function( message ) {
					return that.getHHR( _record.message.hhr );
				})
				.then( function( hhr ) {
					if( !hhr ) {
						...
						return transmitMsg(hhr.server, _record);
					}
				})
				.then( function( _record ) {
					return updateReceivedMessage(_record);
				})
				.then( resolve )
				.catch( function(err) {
