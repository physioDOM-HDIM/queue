...
function Queue(config) {
	...
	if (check.errors.length) {
		logger.warning("config error", JSON.stringify(check.errors,"",4) );
	...
	this.addMsg = function (message) {
		...
		function resetStatus( msg ) {
			...
			var update = {subject: msg.subject, server: msg.server, status: false};
		...
		return new promise(function (resolve, reject) {
			...
			request(url, function (err, resp, body) {
				if (err) {
					...
					logger.debug( "status", req.statusCode );
					if( req.statusCode === 200 && database.length ) {
	...
	this.relayMsg = function (type, message) {
			...
			return new promise( function( resolve, reject ) {
				request({
						...
						if (err) {
							...
							if (resp.statusCode !== 200) {
								logger.warning("hhr pro ", resp.statusCode);
		...
		function checkSchema(message) {
			return new promise( function(resolve, reject) {
				logger.trace("checkSchema");
				var check = messageSchema.validator.validate( message, { "$ref":"/ReceivedMessage"} );
				if( check.errors.length ) {
		...
		return new promise( function( resolve, reject) {
			...
			checkSchema(message)
				.then( function() {
					return saveReceivedMessage(record);
				})
